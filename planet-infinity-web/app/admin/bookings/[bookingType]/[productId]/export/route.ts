...
export async function GET(request: Request, { params }: { params: Promise<{ bookingType: string; productId: string }> }) {
  ...
  const summaries: unknown[][] = [["Booking reference", "Customer", "Mobile / WhatsApp", "Guests", "Status", "Total EGP", "Paid EGP", "Remaining EGP", "Departure / start", "Booked at", "Notes"]];
  ...
  for (const booking of bookingRows) {
    ...
    summaries.push([booking.booking_number, customer.full_name, customer.phone, booking.guest_count, booking.status, total, paid, Math.max(0, total - paid), booking.scheduled_at, booking.created_at, booking.notes]);
