-- A separate public booking path for trips that opt into per-guest
-- accommodation. Existing trip booking functions stay unchanged.
...
declare
  ...
  accommodation_field jsonb;
  accommodation_answer jsonb;
  accommodation_option jsonb;
  accommodation_units integer := 0;
...
begin
  ...
  select value into accommodation_field
  from jsonb_array_elements(coalesce(trip.booking_form_fields, '[]'::jsonb))
  where value ->> 'type' = 'quantity' and lower(coalesce(value ->> 'quantityUnit','')) = 'accommodation'
  limit 1;
  ...
  if jsonb_typeof(accommodation_answer) <> 'object' then raise exception 'Choose accommodation for every guest'; end if;
  for accommodation_option in select value from jsonb_array_elements(coalesce(accommodation_field -> 'options', '[]'::jsonb)) loop
    if accommodation_answer ? (accommodation_option ->> 'id') then
      if (accommodation_answer ->> (accommodation_option ->> 'id')) !~ '^[0-9]+$' then raise exception 'Invalid accommodation choice'; end if;
      if (accommodation_answer ->> (accommodation_option ->> 'id'))::integer > p_guest_count then raise exception 'Invalid accommodation choice'; end if;
      accommodation_units := accommodation_units + (accommodation_answer ->> (accommodation_option ->> 'id'))::integer;
    end if;
  end loop;
  if accommodation_units <> p_guest_count then raise exception 'Choose accommodation for every guest'; end if;
...
revoke all on function private.submit_public_accommodation_trip_booking(uuid, text, text, text, integer, jsonb, text, text[], boolean) from public, anon, authenticated;

create or replace function public.submit_public_accommodation_trip_booking(
  p_trip_id uuid, p_full_name text, p_email text, p_phone text, p_guest_count integer,
  p_selections jsonb, p_notes text, p_policy_slugs text[], p_whatsapp_opt_in boolean default false
)
returns table (id uuid, booking_number text)
...
as $$
  select * from private.submit_public_accommodation_trip_booking(
    p_trip_id, p_full_name, p_email, p_phone, p_guest_count,
    p_selections, p_notes, p_policy_slugs, p_whatsapp_opt_in
  );
$$;

revoke all on function public.submit_public_accommodation_trip_booking(uuid, text, text, text, integer, jsonb, text, text[], boolean) from public, anon, authenticated;
grant execute on function public.submit_public_accommodation_trip_booking(uuid, text, text, text, integer, jsonb, text, text[], boolean) to anon, authenticated;
